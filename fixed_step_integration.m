...
fixed_step_integration(rate_func_in,step_func,tspan,X0,h_ref)
    ...
    % step_func: function uses for integration (forward euler, backward
    % euler, implicit midpoint, or explicit midpoint)
    ...
    for i = 2:N
        [X_next, eval] = step_func(rate_func_in, t_list(i), X_list(:,i-1), h_avg);
        X_list(:,i) = X_next;
